...
| **Email Address**           | כן (`required`) | **בדיקה מקצועית** (`emailValidation.js`): trim, אורך ≤254, regex לפי תקן, חשבון ≤64 תווים, אין `..` בדומיין, TLD ≥2 תווים. חל גם ב-Login וב-Forgot Password. |
...
| שדה               | חובה            | בדיקות                                                                           |
| ----------------- | --------------- | -------------------------------------------------------------------------------- |
| **Email Address** | כן (`required`) | `isValidEmail()` – בדיקה מקצועית. שליחה ל־`supabase.auth.resetPasswordForEmail`. |
