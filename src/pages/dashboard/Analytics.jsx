...
const StatCard = ({ title, value, change, icon, trend }) => (
  <div className="bg-[#1e152f] border border-[#584674] rounded-2xl p-6 relative overflow-hidden group hover:border-[#6b5a87] transition-colors">
    <div className="flex justify-between items-start mb-4">
      ...
      {change && (
        <span
          className={`text-xs font-bold px-2 py-1 rounded-full ${trend === 'up' ? 'text-green-400 bg-green-400/10' : 'text-red-400 bg-red-400/10'}`}
        >
          {change}
        </span>
      )}
...
const Analytics = () => {
  ...
  const fetchStats = async () => {
    try {
      ...
      console.log('Analytics - User found:', user.id);
      ...
      console.log('Analytics - Clicks fetch result:', {
        count: allClicks?.length,
        error: clicksError,
      });
  ...
  return (
    <div className="flex flex-col gap-8 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
        <StatCard title="Total Clicks" value={formatNumber(stats.totalClicks)} icon="ads_click" />
        <StatCard title="Bot Detected" value={formatNumber(stats.botDetected)} icon="security" />
        <StatCard title="Active Links" value={formatNumber(stats.activeLinks)} icon="link" />
      </div>
