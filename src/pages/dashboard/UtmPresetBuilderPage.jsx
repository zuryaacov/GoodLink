...
const getPlatformLogo = (platform) => {
  switch (platform) {
    ...
    case 'instagram':
      return (
        <div className="w-12 h-12 rounded-lg flex items-center justify-center overflow-hidden bg-gradient-to-br from-[#f9ed32] via-[#ee2a7b] to-[#6228d7] p-0.5">
...
const PLATFORMS = {
  ...
  snapchat: { id: 'snapchat', name: 'Snapchat', color: 'yellow' },
};
...
const UtmPresetBuilderPage = () => {
  ...
  const handleSave = async () => {
    try {
      ...
      if (id) {
        const { error } = await supabase.from('utm_presets').update(presetData).eq('id', id);
        ...
        <div>
          ...
          <div className="bg-slate-950 border border-slate-800 rounded-2xl p-6">
                      ...
                      <span
                        className={
                          value.includes('{') || value.includes('__')
                            ? 'font-bold underline decoration-slate-600'
                            : ''
                        }
                      >
                        {value}
            ...
            <p className="text-xs text-slate-500 mt-3">
              Preview shows UTM parameters. This preset will be linked to a specific link when
              editing/adding a link.
            </p>
          </div>
        </div>
