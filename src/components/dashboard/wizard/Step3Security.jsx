...
const botActionOptions = [
  {
    ...
    label: 'Allow',
    icon: '✅',
    description: 'Allow bots through without tracking',
  },
  ...
  const handleAddGeoRule = () => {
    ...
    if (editingRuleIndex !== null) {
      ...
      updatedRules[editingRuleIndex] = {
        ...
        url: urlValidation.normalizedUrl || newGeoRule.url,
      };
      ...
      updateFormData('geoRules', [
        ...currentRules,
        { country: newGeoRule.country, url: urlValidation.normalizedUrl || newGeoRule.url },
      ]);
    }
  ...
  return (
    ...
    >
      ...
      <div className="text-center">
        <h3 className="text-xl sm:text-2xl font-bold text-[#1b1b1b] mb-2">Security & Logic</h3>
      ...
      <div>
        ...
        {formData.botAction === 'redirect' && (
          <motion.div
            ...
            className="mt-4 p-4 bg-white border border-slate-200 rounded-xl"
          >
            ...
            <div className="relative">
              <input
                ...
                className={`w-full px-4 py-3 bg-white border-2 rounded-xl text-[#1b1b1b] placeholder-slate-500 focus:outline-none transition-colors ${
                  fallbackUrlError
            ...
            {fallbackUrlError && <p className="text-red-400 text-xs mt-2">{fallbackUrlError}</p>}
        ...
        <div className="space-y-3 sm:space-y-4">
          ...
          >
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[#1b1b1b] font-medium text-sm sm:text-base mb-1">
                  All other countries
                </p>
              ...
              {formData.geoRules.map((rule, index) =>
                ...
                editingRuleIndex === index ? null : (
                  <motion.div
                    ...
                    initial={{ opacity: 0, x: -20 }}
                    animate={{ opacity: 1, x: 0 }}
                    className="relative p-4 sm:p-5 bg-white border border-slate-200 rounded-xl hover:border-primary/30 transition-colors"
                  >
                    {/* Delete Button - Top Right */}
                    <button
                      onClick={() => handleRemoveGeoRule(index)}
                      className="absolute top-3 right-3 text-[#a855f7] hover:text-[#9333ea] transition-colors p-1.5"
                      title="Delete rule"
                    >
                    ...
                    <div className="pr-10">
                      <div className="flex items-start gap-3 mb-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-[#1b1b1b] font-medium text-sm sm:text-base mb-2">
                            Country is
                          </p>
                          <button
                            onClick={() => handleEditGeoRule(index)}
                            className="flex items-center gap-2 px-3 py-2 bg-white border border-slate-200 rounded-lg hover:border-primary transition-colors w-full text-left"
                          >
                            <span className="text-2xl">{getCountryFlag(rule.country)}</span>
                            <span className="text-[#1b1b1b] font-medium flex-1">
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <input
                            ...
                            value={rule.url}
                            readOnly
                            className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-[#1b1b1b] text-xs sm:text-sm font-mono truncate focus:outline-none"
                            onClick={() => handleEditGeoRule(index)}
                          />
                        </div>
                      </div>
          ...
          {showGeoRuleForm ? (
            <motion.div
              initial={{ opacity: 0, height: 0 }}
              animate={{ opacity: 1, height: 'auto' }}
              className="p-4 sm:p-5 bg-white border-2 border-primary/50 rounded-xl space-y-4"
            >
              <div className="relative" ref={countryPickerRef}>
                <label className="block text-sm font-medium text-[#1b1b1b] mb-2">If Country is</label>
                <button
                  onClick={() => setShowCountryPicker(!showCountryPicker)}
                  className={`w-full flex items-center gap-3 px-4 py-3 bg-white border rounded-xl transition-colors text-left ${
                    newGeoRule.country
                      ? 'border-primary'
                ...
                >
                  {newGeoRule.country ? (
                    <>
                      ...
                      <span className="text-[#1b1b1b] font-medium flex-1">
                        {getCountryName(newGeoRule.country)}
                      </span>
                  ...
                  <span
                    className={`material-symbols-outlined text-slate-500 transition-transform ${showCountryPicker ? 'rotate-180' : ''}`}
                  >
                    arrow_drop_down
                  </span>
                </button>
                ...
                {/* Dropdown Menu */}
                {showCountryPicker && (
                  <div className="absolute z-50 w-full mt-2 bg-white border border-slate-200 rounded-xl shadow-2xl max-h-64 overflow-hidden flex flex-col">
                    {/* Search Input */}
                    <div className="p-3 border-b border-slate-200">
                      <div className="relative">
                        ...
                        <input
                          ...
                          className="w-full pl-10 pr-4 py-2 bg-white border border-slate-200 rounded-lg text-[#1b1b1b] placeholder-slate-500 focus:outline-none focus:border-primary transition-colors text-sm"
                          autoFocus
                        />
                      </div>
                    </div>
              ...
              <div>
                <label className="block text-sm font-medium text-[#1b1b1b] mb-2">Then go to URL</label>
                <input
                  type="url"
                  ...
                  className={`w-full px-4 py-3 bg-white border rounded-xl text-[#1b1b1b] text-sm placeholder-slate-500 focus:outline-none transition-colors ${
                    geoRuleErrors.url
                      ? 'border-red-500 focus:border-red-500'
                      : 'border-slate-200 focus:border-primary'
                  }`}
              ...
              <div className="flex gap-2 sm:gap-3">
                <button
                  ...
                  className="flex-1 px-4 py-2.5 sm:py-3 bg-[#a855f7] hover:bg-[#9333ea] disabled:opacity-50 disabled:cursor-not-allowed text-white font-bold rounded-xl transition-colors text-sm sm:text-base"
                >
                ...
                <button
                  ...
                  className="px-4 sm:px-6 py-2.5 sm:py-3 bg-slate-200 hover:bg-slate-300 text-[#1b1b1b] font-medium rounded-xl transition-colors text-sm sm:text-base"
                >
                  Cancel
                </button>
              </div>
            ...
            <button
              ...
              className="w-full px-4 py-3 sm:py-4 border-2 border-dashed border-slate-300 text-slate-500 hover:text-[#1b1b1b] hover:border-primary rounded-xl transition-colors text-sm sm:text-base font-medium flex items-center justify-center gap-2"
            >
