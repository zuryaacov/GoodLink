  ...
  return (
    ...
    >
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-3 rounded-xl border transition-colors ${
                formData.platformPreset === preset.name
                  ...
                  : 'border-slate-200 bg-white text-[#1b1b1b] hover:border-slate-300'
              }`}
            >
      ...
      <div className="space-y-4">
        <div>
          <label className="block text-sm font-medium text-[#1b1b1b] mb-2">UTM Source</label>
          <input
            type="text"
        ...
        <div>
          <label className="block text-sm font-medium text-[#1b1b1b] mb-2">UTM Medium</label>
          <input
            type="text"
        ...
        <div>
          <label className="block text-sm font-medium text-[#1b1b1b] mb-2">UTM Campaign</label>
          <input
            type="text"
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 p-4 bg-white border border-slate-200 rounded-xl">
        ...
        <button
          ...
          className={`relative w-14 h-7 sm:w-12 sm:h-6 rounded-full transition-colors flex-shrink-0 ${
            formData.parameterPassThrough ? 'bg-[#00F59B]' : 'bg-slate-200'
          }`}
      ...
      {(formData.utmSource || formData.utmMedium || formData.utmCampaign) && (
        <div className="p-4 bg-white border border-slate-200 rounded-xl">
          ...
          <div className="space-y-1 text-sm font-mono">
            {formData.utmSource && <p className="text-[#1b1b1b]">utm_source={formData.utmSource}</p>}
            {formData.utmMedium && <p className="text-[#1b1b1b]">utm_medium={formData.utmMedium}</p>}
            {formData.utmCampaign && (
              <p className="text-[#1b1b1b]">utm_campaign={formData.utmCampaign}</p>
            )}
            {formData.utmContent && <p className="text-[#1b1b1b]">utm_content={formData.utmContent}</p>}
          </div>
        </div>
      )}
    </motion.div>
